...
def generate_hamiltonian_ZGNR(N, k, a, t1, epsilon_list):
    ...
    for i in range(0, N):
        ...
        # Odd case
            ...
            print('generate_hamiltonian_ZGNR: Atom indice error')
            exit()
    ...
    for i in range(0, N):
        ...
        if i == 0:
            H[i, i + 1] = -t1 
            H[i, i + 1] += -t1 * np.conj(phase)
            continue

        # Check RHS edge case
        if (i == (N - 1)) and (is_even(N)):
            # End is on bottom row
            j = (2 * N) - 1
            H[j, j - 1] = -t1 
            H[j, j - 1] += -t1 * np.conj(phase)
            continue
...
def generate_hamiltonian_AGNR(N, k, a, t1, epsilon_list):
    '''
    ...
    for i in range(0, N, 1):
        ...
        # Catch wrong input
            ...
            print('generate_hamiltonian_AGNR: atom indice error')
            exit()
...
def test_hamiltonian_AGNR(type, print_matrix=None):
    ...
    H = generate_hamiltonian_AGNR(N, k1, a, t1, epsilon_list)
...
def eigensolver_AGNR(N, k_vals, a, t1, epsilon_list):
    ...
    for i, k in enumerate(k_vals): 
        ...
        H = generate_hamiltonian_AGNR(N, k, a, t1, epsilon_list)
...
def eigensolver_ZGNR(N, k_vals, a, t1, epsilon_list):
...
def plot_bands(GNR_type, N, k_vals, E_bands):
    ...
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        plt.title('Band Structure of ' + str(N) + '-AGNR: TB NN-Hopping')
    else:
        plt.title('Band Structure of ' + str(N) + '-ZGNR: TB NN-Hopping')
...
def calculate_band_gap(GNR_type, k, N, a, t1, epsilon_list):
...
def plot_band_gaps(GNR_type, N_values, a, t1, epsilon_list): 
    ...
    for N in N_values:
        ...
        band_gap = calculate_band_gap(GNR_type, k, N, a, t1, epsilon_list)
    ...
    if GNR_type in ['ZGNR', 'zgnr', 'Z', 'z']:
        ...
        for N in N_values:
            ...
            band_gap_zgnr = calculate_band_gap(GNR_type, k, N, a, t1, epsilon_list)
        ...
        plt.title('ZGNR: Band Gap at k=pi/a as a function of Ribbon Width N')
        plt.grid(True)
        plt.show()
    ...
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        ...
        metallic_band_gaps = []
        non_metallic_N = [] 
        non_metallic_band_gaps = []
        
        metallic_N_values = []
        for p in range(0, N, 1):
            N_A = 3 * p + 2
            metallic_N_values.append(N_A)

        for i, N in enumerate(N_values):
            if N in metallic_N_values:
                metallic_N.append(N)
                metallic_band_gaps.append(band_gaps[i])
            else: 
                non_metallic_N.append(N)
                non_metallic_band_gaps.append(band_gaps[i])

        # Plot at k=0
        plt.figure(dpi=200)
        plt.scatter(non_metallic_N, non_metallic_band_gaps, edgecolors='black', facecolors='none', s=50, label='Non-metallic')
        plt.scatter(metallic_N, metallic_band_gaps, edgecolors='blue', facecolors='none', s=50, label='Metallic')
        plt.xlabel('Ribbon Width (N)')
        plt.ylabel('Band Gap at k=0 (eV)')
        plt.xlim(0, np.max(N_values) + 1)
        plt.xticks(range(0, np.max(N_values) + 2, 2))
        plt.title('AGNR: Band Gap at k=0 as a function of Ribbon Width N')
        plt.grid(True)
        plt.legend()
        plt.show()
    ...
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        ...
        plt.title('ZGNR: Band Gap at k=0 as a function of Ribbon Width N')
...
def main():
    ...
    # Testing
    #test_hamiltonian_AGNR('odd')
    #test_hamiltonian_AGNR('even', print_matrix=True)
    #test_hamiltonian_ZGNR()

    #test_hamiltonian_ZGNR()
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        E_bands = eigensolver_AGNR(N, k, a, t1, epsilon_list)
    elif GNR_type in ['ZGNR', 'zgnr', 'Z', 'z']:
        E_bands = eigensolver_ZGNR(N, k, a, t1, epsilon_list)
    else:
        print('GNR_type Error : ', GNR_type, ' is not a valid input. Use AGNR or ZGNR')
    
    # Plot band structure
    plot_bands(GNR_type, N, k, E_bands)
    
    # Plot band gap
    plot_band_gaps(GNR_type, N_values, a, t1, epsilon_list)
