'''
This program calculates and analyses the band structures of armchair (AGNR) and zigzag (ZGNR) 
graphene nanoribbons (GNRs) using the tight-binding TB model. Users can select 1st, 2nd or 3rd 
nearest-neighbour hopping interactions as well as the width N of the GNR system.

Usage: 
    Execute the code to be prompted for user input.
...
def generate_hamiltonian_ZGNR(N, k, a, epsilon_list, t1, t2, t3):
    ...
    Args:
        ...
        t2 : float, 2nd nearest-neighbour hopping parameter
        t3 : float, 3rd nearest-neighbour hopping paramter
    ...
    for i in range(0, N):
        ...
        if t3 != 0:
            ...
            if i-1 >=  LHS:
                if is_even(i):
                    H[bi, bi-1] += -2 * t3 * np.cos(k*a)
                    H[ti, ti-1] += -t3
                elif is_odd(i):
                    H[bi, bi-1] += -t3 
                    H[ti, ti-1] += -2 * t3 * np.cos(k*a)
...
def generate_hamiltonian_AGNR(N, k, a, epsilon_list, t1, t2, t3):
    ...
    Args:
        ...
        t1 : float, 1st nearest-neighbour hopping parameter
        t2 : float, 2nd nearest-neighbour hopping parameter
        t3 : float, 3rd nearest-neighbour hopping parameter
        epsilon_list : list(float), On-site energies
...
def eigensolver_AGNR(N, k_vals, a, epsilon_list, t1, t2, t3):
    '''
    Solves the Hamiltonian H for the specified AGNR system

    Args:
        N : int, Ribbon width
        k_vals : list(float) : List containing all sampled k-points
        a : float, lattice constant
        epsilon_list : list(float), On-site energies
        t1 : float, 1st nearest-neighbour hopping parameter
        t2 : float, 2nd nearest-neighbour hopping parameter
...
def eigensolver_ZGNR(N, k_vals, a, epsilon_list, t1, t2, t3):
    '''
    Solves the Hamiltonian H for the specified ZGNR system
...
def plot_bands(GNR_type, NN_type, N, k_vals, E_bands):
    ...
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        plt.xlabel(r'$k$  $(1/a)$', fontsize=14)
    else:
        plt.xlabel(r'$k$  $(1/z)$', fontsize=14)
...
def calculate_band_gap(GNR_type, k, N, a, epsilon_list, t1, t2, t3):
    '''
    Determines the band gap value for the given system.
    '''
    # AGNR selected
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        H = generate_hamiltonian_AGNR(N, k, a, epsilon_list, t1, t2, t3)
    # ZGNR selected
        ...
        H = generate_hamiltonian_ZGNR(N, 0, a, epsilon_list, t1, t2, t3)
    else:
...
def plot_band_gaps(GNR_type, NN_type, N_values, a, epsilon_list, t1, t2, t3): 
    '''
    Plots band gaps for the calculated system.
    '''
    # If ZGNR  calculate band gap at k = pi/a
    if GNR_type in ['ZGNR', 'zgnr', 'Z', 'z']:
        ...
        plt.xlabel(r'Ribbon Width ($N_z$)', fontsize=14)
        plt.ylabel(r'Band Gap at $k=0$ (eV)', fontsize=14)
    ...
    if GNR_type in ['AGNR', 'agnr', 'A', 'a']:
        ...
        for N in N_values:
            ...
            band_gap_agnr = calculate_band_gap(GNR_type, k, N, a, epsilon_list, t1, t2, t3)
            band_gaps_agnr.append(band_gap_agnr)
        ...
        N_end = len(N_values) - 1

        # Generate values for Na=3p, Na=3p+1, Na=3p+2
        for p in range(0, N_end, 1):
            if 3 * p in N_values:
                N_3p.append(3 * p)
        ...
        for i, N in enumerate(N_values):
            ...
            # print('i:', i)
            if N in N_3p:
                band_gaps_3p.append(band_gaps_agnr[i])
            elif N in N_3p1:
                band_gaps_3p1.append(band_gaps_agnr[i])
            elif N in N_3p2:
                band_gaps_3p2.append(band_gaps_agnr[i])
        ...
        plt.xlabel(r'Ribbon Width ($N_A$)', fontsize=14)
        plt.ylabel(r'Band Gap at $k=0$ (eV)', fontsize=14)
        plt.xlim(0, np.max(N_values) + 1)
        plt.xticks(range(0, np.max(N_values) + 2, 2))
